...
export async function verifyRepoWriteAccess(token) {
  const res = await githubFetch(token, `/repos/${REPO_FULL_NAME}`);
  if (!res.ok) {
    return { ok: false, status: res.status, error: "Unable to verify repository permission." };
  }
  const data = await res.json();
  ...
  if (permissions.admin === true || permissions.push === true) {
    return {
      ok: true,
      permission: permissions.admin === true ? "admin" : "write",
    };
  }
...
export async function requireAdmin(context, options = {}) {
  ...
  if (needsPermissionCheck) {
    let permission;
    try {
      permission = await verifyRepoWriteAccess(session.githubToken);
    } catch {
